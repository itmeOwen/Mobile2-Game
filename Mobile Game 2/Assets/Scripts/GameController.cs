...
public class GameController : MonoBehaviour
{
    ...
    private int obstacleSpawnNum;
    public Transform egg;
    private int eggSpawnNum;
    ...
    private void SpawnObstacle(Transform newTile)
    {
        ...
        {
            obstacleSpawnNum = Random.Range(0, obstacleSpawnPoints.Count);
            // Get a random object from the ones we have
            var spawnPoint = obstacleSpawnPoints[obstacleSpawnNum];
            ...
            var eggSpawnPoint = obstacleSpawnPoints[eggSpawnNum];
            var eggSpawnPos = eggSpawnPoint.transform.position;
            var newEgg = Instantiate(egg, eggSpawnPos,
            Quaternion.identity);
            newEgg.SetParent(eggSpawnPoint.transform);

            //obstacleSpawnNum = null;
            //eggSpawnNum = null;

            obstacleSpawnPoints = null;
    ...
    private void RandomEggSpawnPoint() //no zhuang number
    {
        eggSpawnNum = Random.Range(0, 3);

        if(eggSpawnNum == obstacleSpawnNum)
        {
            RandomEggSpawnPoint();
        }
    }
